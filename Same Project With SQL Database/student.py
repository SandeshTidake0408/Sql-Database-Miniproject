...
class StudentClass:
    def __init__(self, root):
        ...
        self.search_roll = Entry(self.root, textvariable=self.var_search, font=(
            "Mukta", 12),  bg="#FFFACA", fg="black").place(x=930, y=75, width=180)
...
# ==============db function==============
    ...
    def search(self):
        ...
        cursor = con.cursor()
        try:
            cursor.execute("SELECT * FROM student WHERE roll = %s",
                           (self.var_search.get(),))
        ...
        try:
            if self.var_roll.get() == "":
                ...
                if row != None:
                    ...
                    cursor.execute("INSERT INTO student (roll, name, email, gender, dob, contact, admission, course, state, city, pincode, address) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (
                        self.var_roll.get(),
                        self.var_name.get(),
                        self.var_email.get(),
                        self.var_gender.get(),
        ...
        try:
            if self.var_roll.get() == "":
                ...
                if row == None:
                    ...
                    cursor.execute("UPDATE student SET name = %s, email = %s, gender = %s, dob = %s, contact = %s, admission = %s, course = %s, state = %s, city = %s, pincode = %s, address = %s WHERE roll = %s", (
                        self.var_name.get(),
                        self.var_email.get(),
                        self.var_gender.get(),
                        self.var_dob.get(),
